...
func (c *Cache) SetTypeMtime(ctx context.Context, clientID string, dataType int, mtime int64) {
	...
	fmt.Println("[SetTypeMtime]", "key:", key, "val:", val)
}
...
func (c *Cache) IsTypeMtimeUpdated(ctx context.Context, clientID string, dataType int, token int64) bool {
	...
	if cachedTokenStr == "" {
		fmt.Println("[IsTypeMtimeUpdated] cache misses for key:", key)
		return true
	}

	// Token parsing is unlikely, but if happens, fall through to querying
	// dynamoDB.
	...
	if cachedToken > token {
		fmt.Println("[IsTypeMtimeUpdated] cache hit and cachedToken > clientToken, has updates, cachedToken:", cachedToken, "clientToken:", token, "key:", key)
		return true
	}

	fmt.Println("[IsTypeMtimeUpdated] cache hit with cachedToken <= clientToken, no updates, cachedToken:", cachedToken, "clientToken:", token, "key:", key)
